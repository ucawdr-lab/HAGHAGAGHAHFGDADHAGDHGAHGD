local loadstring = function(...)
	...
	if err and vape then
		vape:CreateNotification('GRIEFER', 'Failed to load : '..err, 30, 'alert')
	end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/ucawdr-lab/HAGHAGAGHAHFGDADHAGDHGAHGD/'..readfile('newvape/profiles/commit.txt')..'/'..select(1, path:gsub('newvape/', '')), true)
		end)
...
local function serverHop(pointer, filter)
	...
	if not pointer then
		notif('GRIEFER', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('GRIEFER', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('GRIEFER', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('GRIEFER', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end
	...
	function whitelist:update(first)
		local suc = pcall(function()
			local _, subbed = pcall(function()
				return game:HttpGet('https://github.com/ucawdr-lab/HAGHAGAGHAHFGDADHAGDHGAHGD')
			end)
			local commit = subbed:find('currentOid')
			commit = commit and subbed:sub(commit + 13, commit + 52) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/ucawdr-lab/HAGHAGAGHAHFGDADHAGDHGAHGD/'..commit..'/PlayerWhitelist.json', true)
		end)
...
run(function()
    ...
    local function ChangeFFlag(suc)
    	...
    	if not success or typeof(json) ~= 'table' then
    		notif('GRIEFER', 'Invalid json format for fflag', 12, 'warning')
    		return
    	end
    	...
    	notif('GRIEFER', 'FFlags applied, Go in a new game to take effect', 12, 'info')
    end
    ...
    FFlag = vape.Legit:CreateModule({
    	...
    	Function = function(call)
    		if call then
    			...
    			notif('GRIEFER', 'Inorder to disable fflags you have applied, You need to restart roblox', 20, 'info')
    		end
    	end,
    })
